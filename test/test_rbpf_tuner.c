 ...
 *
 * Usage:
 *   test_rbpf_tuner [fast|normal|detailed|middle|balanced]
 *
 * Modes:
 *   fast     - Reduced grid, quick exploration (~2 min)
 *   normal   - Default search (~10 min)
 *   detailed - Fine grid, thorough search (~30 min)
 *   middle   - Heavy bonus for R1/R2 accuracy
 *   balanced - Asymmetric gaps, prevents sacrificing extreme regimes
 */
...
static void synthetic_data_print_stats(const SyntheticData *data, int n_regimes)
{
    ...
    {
        float avg_vol = regime_counts[r] > 0 ? regime_vol_sum[r] / regime_counts[r] : 0;
...
int main(int argc, char *argv[])
{
    ...
    {
        cfg = tuner_config_balanced();
        printf("  Mode: BALANCED (asymmetric gaps, min-regime penalty)\n");
    }
    else
    {
