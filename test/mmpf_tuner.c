    ...
    double base_stickiness)
{
    ...
    metrics.avg_transition_lag = (transition_count > 0) ? (double)total_lag / transition_count : 0.0;
    metrics.false_crisis_count = (double)false_crisis;

    for (int s = 0; s < SCENARIO_COUNT; s++)
    {
        metrics.per_scenario_accuracy[s] = (scenario_count[s] > 0) ? 100.0 * scenario_correct[s] / scenario_count[s] : 0.0;
    }
