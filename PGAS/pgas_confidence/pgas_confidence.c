...
/* Include PGAS header for state access (skip if testing without MKL) */
...
static float level_to_gamma(PGASConfidenceLevel level, const PGASConfidenceConfig *cfg) {
    switch (level) {
        case PGAS_CONFIDENCE_VERY_LOW:  return cfg->gamma_very_low;
        case PGAS_CONFIDENCE_LOW:       return cfg->gamma_low;
        case PGAS_CONFIDENCE_MEDIUM:    return cfg->gamma_medium;
        case PGAS_CONFIDENCE_HIGH:      return cfg->gamma_high;
        case PGAS_CONFIDENCE_VERY_HIGH: return cfg->gamma_very_high;
        default:                        return cfg->gamma_medium;
    }
}
...
int pgas_confidence_compute(
    ...
    conf->compute_time_ns = (end.tv_sec - start.tv_sec) * 1000000000LL +
...
const char* pgas_confidence_level_str(PGASConfidenceLevel level) {
    switch (level) {
        case PGAS_CONFIDENCE_VERY_LOW:  return "VERY_LOW";
        case PGAS_CONFIDENCE_LOW:       return "LOW";
        case PGAS_CONFIDENCE_MEDIUM:    return "MEDIUM";
        case PGAS_CONFIDENCE_HIGH:      return "HIGH";
        case PGAS_CONFIDENCE_VERY_HIGH: return "VERY_HIGH";
