 ...
 *═══════════════════════════════════════════════════════════════════════════════*/
    ...
    #include <intrin.h>
    #define CBUF_ALIGNED __declspec(align(64))
    #define CBUF_ATOMIC_UINT32 volatile long
    #define CBUF_ATOMIC_LOAD(ptr) _InterlockedCompareExchange((ptr), 0, 0)
    #define CBUF_ATOMIC_STORE(ptr, val) _InterlockedExchange((ptr), (val))
    #define CBUF_ATOMIC_INIT(val) (val)
#else
    /* GCC/Clang: Use C11 atomics */
    #include <stdatomic.h>
    #define CBUF_ALIGNED __attribute__((aligned(64)))
    #define CBUF_ATOMIC_UINT32 _Atomic uint32_t
    #define CBUF_ATOMIC_LOAD(ptr) atomic_load(ptr)
    #define CBUF_ATOMIC_STORE(ptr, val) atomic_store((ptr), (val))
    #define CBUF_ATOMIC_INIT(val) (val)
#endif
 ...
 */
typedef struct {
    CBUF_ALIGNED ObservationEntry data[CBUF_CAPACITY];
    
    /** Write position (main thread only) */
    CBUF_ATOMIC_UINT32 head;
    
    /** Read position (background thread only) */
    CBUF_ATOMIC_UINT32 tail;
