...
{
    if (!buf || !entry || cbuf_is_empty(buf)) return false;
                    ...
                    uint64_t *consumed)
{
    ...
    if (pushed)   *pushed = buf->total_pushed;
    if (dropped)  *dropped = buf->total_dropped;
    if (consumed) *consumed = buf->total_consumed;
}

uint64_t cbuf_get_staleness(const CircularBuffer *buf, uint64_t current_tick)
{
    if (!buf || cbuf_is_empty(buf)) return 0;
    
    ObservationEntry oldest;
    if (!cbuf_peek_oldest(buf, &oldest)) return 0;
