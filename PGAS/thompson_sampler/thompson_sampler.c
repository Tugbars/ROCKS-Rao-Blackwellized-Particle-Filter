...
void thompson_sampler_dirichlet(
    ...
    if (!sampler || !alpha || !out || K <= 0) return;
    ...
    for (int i = 0; i < K; i++) {
        float a = alpha[i];
        if (a < min_alpha) a = min_alpha;  /* Ensure valid concentration */
...
static void apply_floor_and_normalize(float *row, int K, float floor_prob) {
    ...
    if (n_floored == 0) {
        ...
        if (sum > 1e-10f && fabsf(sum - 1.0f) > 1e-6f) {
            float inv_sum = 1.0f / sum;
            for (int i = 0; i < K; i++) row[i] *= inv_sum;
        }
        return;
    }
    ...
    if (available_mass < floor_prob || non_floored_sum < 1e-10f) {
        ...
        float uniform = 1.0f / K;
        for (int i = 0; i < K; i++) row[i] = uniform;
        return;
    }
    ...
    for (int i = 0; i < K; i++) {
        if (row[i] < floor_prob) {
            ...
            if (row[i] < floor_prob) row[i] = floor_prob;
        }
    }
...
ThompsonSampleResult thompson_sampler_sample(
    ...
    if (should_explore) {
        ...
        for (int i = 0; i < K; i++) {
            ...
            if (row_sum < 1e-10f) row_sum = 1e-10f;
...
float thompson_sampler_get_explore_ratio(const ThompsonSampler *sampler) {
    if (!sampler || sampler->total_samples == 0) return 0.0f;
