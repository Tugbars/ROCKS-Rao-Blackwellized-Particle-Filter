...
static void scenario_regime_transitions(void) {
    ...
    typedef struct { int start; int end; float vol; const char *name; } Regime;
    Regime regimes[] = {
        {0,    300,  0.005f, "Calm"},
        ...
        if (res.should_trigger) {
            ...
            for (int i = 0; i < n_transitions; i++) {
                if (detections[i].detected_tick == -1 &&
                    ...
                    t < transition_ticks[i] + 200) {  /* Within 200 ticks */
...
static void scenario_false_positive_rate(void) {
    ...
    float fp_rate = (float)triggers / ((N - cfg.warmup_ticks) / 1000.0f);
    
    printf("\n  SUMMARY:\n");
    printf("    Duration: %d ticks (%.1f after warmup)\n", N, (float)(N - cfg.warmup_ticks));
...
static void scenario_spike_filtering(void) {
    ...
    for (int t = 0; t < N; t++) {
        ...
        if (t >= 200 && t < 250) {
            if (res.surprise_sigma > max_surprise) max_surprise = res.surprise_sigma;
    ...
    for (int t = 0; t < N; t++) {
        ...
        if (res.should_trigger) {
            if (t >= 200 && t < 400) {
                ...
                if (first_trigger_tick < 0) first_trigger_tick = t;
