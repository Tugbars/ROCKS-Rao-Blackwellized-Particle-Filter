...
/*═══════════════════════════════════════════════════════════════════════════════
 * ATOMIC STATE HELPERS (lock-free hot-path!)
 *═══════════════════════════════════════════════════════════════════════════════*/
...
LifeboatTrigger lifeboat_check_trigger(LifeboatManager *mgr,
                                        ...
                                        float kl)
{
    if (!mgr) return LIFEBOAT_TRIGGER_NONE;
...
bool lifeboat_start_run(LifeboatManager *mgr,
                        ...
                        const float *ref_h)
{
    if (!mgr || !observations || count < 2) return false;
...
{
    if (!mgr || mgr->ready_cloud < 0) return 0;
...
{
    if (!mgr || mgr->ready_cloud < 0) return NULL;
...
bool lifeboat_inject(LifeboatManager *mgr,
                     ...
                     uint64_t *rng_state)
{
    if (!mgr || !rbpf_regimes || !rbpf_h || !rbpf_weights) return false;
...
void lifeboat_fast_forward(const LifeboatCloud *cloud,
                           ...
                           uint64_t *rng_state)
{
    if (!cloud || !regimes || !h || !weights || n_ticks <= 0 || !rng_state) return;
...
void lifeboat_print_diagnostics(const LifeboatManager *mgr)
{
    ...
    LifeboatThreadState state = get_thread_state(mgr);
    switch (state) {
        case LIFEBOAT_THREAD_IDLE: state_str = "IDLE"; break;
        case LIFEBOAT_THREAD_RUNNING: state_str = "RUNNING"; break;
        case LIFEBOAT_THREAD_READY: state_str = "READY"; break;
        case LIFEBOAT_THREAD_SHUTDOWN: state_str = "SHUTDOWN"; break;
        default: state_str = "UNKNOWN";
    }
    
    const char *mode_str;
    switch (mgr->config.mode) {
        case LIFEBOAT_MODE_REPLACE: mode_str = "REPLACE"; break;
        case LIFEBOAT_MODE_MIX: mode_str = "MIX"; break;
        case LIFEBOAT_MODE_RESAMPLE: mode_str = "RESAMPLE"; break;
