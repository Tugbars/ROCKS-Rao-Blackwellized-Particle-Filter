...
static int test_tempering_distribution(void) {
    ...
    for (int r = 2; r <= 3; r++) {
        if (counts[r] < min_non) min_non = counts[r];
        if (counts[r] > max_non) max_non = counts[r];
    }
...
static int test_regime_distribution(void) {
    ...
    for (int t = 0; t < 40; t++) rbpf_trajectory_record(&traj, 0, -3.0f);
    for (int t = 0; t < 30; t++) rbpf_trajectory_record(&traj, 1, -3.0f);
    for (int t = 0; t < 20; t++) rbpf_trajectory_record(&traj, 2, -3.0f);
    for (int t = 0; t < 10; t++) rbpf_trajectory_record(&traj, 3, -3.0f);
