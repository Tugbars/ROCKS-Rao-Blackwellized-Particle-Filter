...
static int sample_categorical_log(const double *log_weights, int n, uint64_t *rng)
{
    ...
    for (int i = 1; i < n; i++)
    {
        if (log_weights[i] > max_log)
            max_log = log_weights[i];
    }
...
double pgas_csmc_sweep(PGASState *pgas)
{
    ...
    pgas->acceptance_rate = (pgas->ancestor_proposals > 0) ? (double)pgas->ancestor_accepts / pgas->ancestor_proposals : 0.0;
...
void paris_get_smoothed_particles(const PGASState *pgas,
                                  ...
                                  double *weights)
{
    if (!pgas || !pgas->smoothing_done || t < 0 || t >= pgas->T)
        return;
    ...
    for (int n = 0; n < N; n++)
    {
        ...
        if (regimes)
            regimes[n] = pgas->regimes[t][idx];
        if (h)
            h[n] = pgas->h[t][idx];
        if (weights)
            weights[n] = unif_weight; /* Uniform after smoothing */
    }
}
...
void pgas_print_diagnostics(const PGASState *pgas)
{
    ...
    printf("Acceptance rate:    %.3f %s\n", pgas->acceptance_rate,
           pgas->acceptance_rate >= PGAS_TARGET_ACCEPTANCE ? "(CONVERGED)" : pgas->acceptance_rate >= PGAS_ABORT_ACCEPTANCE ? "(MIXING)"
                                                                                                                            : "(STUCK)");
