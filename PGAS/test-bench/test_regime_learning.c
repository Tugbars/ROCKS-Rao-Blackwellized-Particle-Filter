...
static void pgas_enforce_mu_ordering_single(PGASMKLState *pgas)
{
    ...
    for (int k = 0; k < K; k++) {
        if (perm[k] != k) { already_sorted = 0; break; }
    }
...
int main(int argc, char **argv)
{
    ...
    for (int t = 0; t < T_DATA; t++) obs_d[t] = data.y[t];
    for (int i = 0; i < 16; i++) init_trans[i] = 0.85 * (i % 5 == 0 ? 1 : 0) + 0.05;
    for (int k = 0; k < 4; k++) {
