...
int main(void)
{
    ...
    /* Model parameters */
    const float mu_vol[4] = {-4.50f, -3.67f, -2.83f, -2.00f};
    const float sigma_vol[4] = {0.08f, 0.267f, 0.453f, 0.64f};
    const float phi = 0.97f;
    const float sigma_h = 0.15f;
    ...
    PGASMKLState *pgas_A = pgas_mkl_alloc(K, N, T, SEED);
    if (!pgas_A) {
    ...
    /* Configure */
    pgas_mkl_set_volatility_params(pgas_A, mu_vol, sigma_vol, K);
    pgas_mkl_set_ar_params(pgas_A, phi, sigma_h);
    ...
    pgas_mkl_load_observations(pgas_A, data.y, T);
    pgas_mkl_initialize_reference(pgas_A, data.y, T);
    ...
    PGASMKLState *pgas_B = pgas_mkl_alloc(K, N, T, SEED);
    if (!pgas_B) {
    ...
    /* Same configuration */
    pgas_mkl_set_volatility_params(pgas_B, mu_vol, sigma_vol, K);
    pgas_mkl_set_ar_params(pgas_B, phi, sigma_h);
    ...
    pgas_mkl_load_observations(pgas_B, data.y, T);
    pgas_mkl_initialize_reference(pgas_B, data.y, T);
