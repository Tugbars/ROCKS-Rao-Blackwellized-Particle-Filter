...
{
    if (!pgas) return NULL;
    if (n_trajectories < 1) n_trajectories = PGAS_PARIS_DEFAULT_TRAJECTORIES;
    ...
    state->paris = paris_mkl_alloc(pgas->N, pgas->T, pgas->K, pgas->seed + 5000);
    if (!state->paris) {
        free(state);
        return NULL;
    }
    ...
    if (!state->h_double || !state->weights_double) {
        if (state->h_double) mkl_free(state->h_double);
        if (state->weights_double) mkl_free(state->weights_double);
...
{
    if (!state || !state->pgas || !state->paris) return;
...
{
    if (!state || !state->pgas) return;
    
    PGASMKLState *pgas = state->pgas;
    const int K = pgas->K;
    const float alpha = pgas->trans_prior_alpha;
    const float kappa = pgas->sticky_kappa;
    
    VSLStreamStatePtr stream = (VSLStreamStatePtr)pgas->rng_stream;
    ...
    if (expected_switch_rate > 0.001f) {
        pgas->chatter_ratio = actual_switch_rate / expected_switch_rate;
    }
    ...
    if (pgas->adaptive_kappa_enabled) {
        ...
        pgas->rls_chatter_ema = lambda * pgas->rls_chatter_ema + 
                                (1.0f - lambda) * pgas->chatter_ratio;
        
        /* Adjust κ based on smoothed chatter */
        float smoothed_chatter = pgas->rls_chatter_ema;
...
{
    if (!state || !state->pgas || !state->paris) return 0.0f;
...
{
    if (!ens || !out || ens->n_samples == 0) return;
...
{
    if (!ens || ens->n_samples < 2) return 0.0f;
