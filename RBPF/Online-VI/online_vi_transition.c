...
void online_vi_set_rho_bounds(OnlineVI *vi, double rho_min, double rho_max)
{
    ...
    if (vi->rho_min > vi->rho_max) vi->rho_min = vi->rho_max;
}
...
{
    if (!vi || !regime_probs || !regime_liks) return;
...
{
    if (!vi || !regime_probs || !log_regime_liks) return;
    ...
    for (int j = 1; j < K; j++) {
        if (log_regime_liks[j] > max_log) max_log = log_regime_liks[j];
    }
...
{
    if (!vi || from < 0 || from >= vi->K || to < 0 || to >= vi->K) return 0.0;
...
{
    if (!vi || !row || from < 0 || from >= vi->K) return;
...
double online_vi_get_confidence(OnlineVI *vi)
{
    ...
    double alpha_uniform[ONLINE_VI_MAX_REGIMES];
    for (int i = 0; i < vi->K; i++) {
        alpha_uniform[i] = 1.0;
    }
    double H_max = dirichlet_entropy(alpha_uniform, vi->K) * vi->K;
    
    if (H_max < VI_EPS) return 1.0;
    
    double confidence = 1.0 - vi->total_entropy / H_max;
...
void online_vi_adapt_rho_from_hdp(OnlineVI *vi, double correction_kl, 
                                   double sensitivity)
{
    if (!vi || vi->lr_schedule != VI_LR_ADAPTIVE) return;
...
void online_vi_print(const OnlineVI *vi)
{
    ...
    printf("  t = %lu updates\n", (unsigned long)vi->t);
    printf("  ρ = %.4f (%s)\n", vi->rho,
           vi->lr_schedule == VI_LR_FIXED ? "fixed" :
           vi->lr_schedule == VI_LR_ROBBINS_MONRO ? "Robbins-Monro" : "adaptive");
