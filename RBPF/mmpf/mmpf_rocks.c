...
static rbpf_real_t log_sum_exp(const rbpf_real_t *log_w, int n) {
    ...
    for (int i = 1; i < n; i++) {
        if (log_w[i] > max_log) max_log = log_w[i];
    }
...
void mmpf_buffer_destroy(MMPF_ParticleBuffer *buf) {
    if (buf) {
        if (buf->_memory) aligned_free_impl(buf->_memory);
        free(buf);
    }
}
...
static void stratified_resample_from_buffer(
    ...
    for (int i = 1; i < n_src; i++) {
        if (src->log_weight[i] > max_log) max_log = src->log_weight[i];
    }
...
void mmpf_destroy(MMPF_ROCKS *mmpf) {
    ...
    for (int k = 0; k < MMPF_N_MODELS; k++) {
        if (mmpf->rbpf[k]) rbpf_ksc_destroy(mmpf->rbpf[k]);
        if (mmpf->learner[k]) {
            param_learn_free(mmpf->learner[k]);
            free(mmpf->learner[k]);
        }
        if (mmpf->buffer[k]) mmpf_buffer_destroy(mmpf->buffer[k]);
        if (mmpf->mixed_buffer[k]) mmpf_buffer_destroy(mmpf->mixed_buffer[k]);
    }
...
void mmpf_step(MMPF_ROCKS *mmpf, rbpf_real_t y, MMPF_Output *output) {
    ...
    for (int k = 0; k < MMPF_N_MODELS; k++) {
        mmpf->log_weights[k] += rbpf_log(mmpf->model_likelihood[k] + RBPF_EPS);
    }
    log_weights_to_linear(mmpf->log_weights, mmpf->weights, MMPF_N_MODELS);
    
    /* Renormalize log_weights to prevent underflow over time */
    ...
    /* 9. Get OCSN outlier fraction from dominant model */
    ...
    /* Get outlier fraction from cached output */
    mmpf->outlier_fraction = mmpf->model_output[dom].outlier_fraction;
...
void mmpf_print_summary(const MMPF_ROCKS *mmpf) {
    ...
    printf("Dominant: %s (prob=%.4f)\n",
           mmpf->dominant == MMPF_CALM ? "Calm" :
           mmpf->dominant == MMPF_TREND ? "Trend" : "Crisis",
...
void mmpf_get_diagnostics(const MMPF_ROCKS *mmpf,
    ...
    if (total_steps) *total_steps = mmpf->total_steps;
    if (regime_switches) *regime_switches = mmpf->regime_switches;
    if (imm_mix_count) *imm_mix_count = mmpf->imm_mix_count;
}
