...
static inline rbpf_real_t sigmoid_discount(
    ...
    rbpf_real_t z)
{
    if (z < RBPF_REAL(-5.0)) return RBPF_REAL(0.0);  /* Avoid exp overflow */
    if (z > RBPF_REAL(10.0)) return af->max_discount; /* Saturate */
    ...
    int dominant_regime)
{
    ...
    if (lambda < af->lambda_floor) lambda = af->lambda_floor;
    if (lambda > af->lambda_ceiling) lambda = af->lambda_ceiling;
    ...
    if (ext->storvik_initialized) {
        ext->storvik.forgetting.lambda = lambda;
    }
}
...
void rbpf_ext_enable_adaptive_forgetting(RBPF_Extended *ext)
{
    ...
    if (ext->storvik_initialized) {
        ext->storvik.forgetting.enabled = 1;
        ext->storvik.forgetting.lambda = af->lambda_per_regime[1];  /* Start at R1 */
    }
}
...
void rbpf_ext_set_regime_lambda(RBPF_Extended *ext, int regime, rbpf_real_t lambda)
{
    ...
    if (lambda < RBPF_REAL(0.9)) lambda = RBPF_REAL(0.9);
    if (lambda > RBPF_REAL(0.9999)) lambda = RBPF_REAL(0.9999);
                                    ...
                                    rbpf_real_t max_discount)
{
    ...
    if (max_discount > RBPF_REAL(0.5)) max_discount = RBPF_REAL(0.5);
    if (max_discount < RBPF_REAL(0.01)) max_discount = RBPF_REAL(0.01);
                                   ...
                                   rbpf_real_t ceiling)
{
    ...
    if (floor < RBPF_REAL(0.8)) floor = RBPF_REAL(0.8);
    if (ceiling > RBPF_REAL(0.9999)) ceiling = RBPF_REAL(0.9999);
    if (floor >= ceiling) floor = ceiling - RBPF_REAL(0.01);
                                      ...
                                      rbpf_real_t signal_alpha)
{
    ...
    if (baseline_alpha < RBPF_REAL(0.001)) baseline_alpha = RBPF_REAL(0.001);
    if (baseline_alpha > RBPF_REAL(0.5)) baseline_alpha = RBPF_REAL(0.5);
    if (signal_alpha < RBPF_REAL(0.001)) signal_alpha = RBPF_REAL(0.001);
    if (signal_alpha > RBPF_REAL(0.5)) signal_alpha = RBPF_REAL(0.5);
                                  ...
                                  rbpf_real_t *max_surprise)
{
    ...
    if (interventions) *interventions = af->interventions;
    if (current_lambda) *current_lambda = af->lambda_current;
    if (max_surprise) *max_surprise = af->max_surprise_seen;
}
