...
#define SOFT_DIRICHLET_MAX_REGIMES 8
    ...
    /**
     * @brief Adapt forgetting factor based on regime entropy
     *
     * When market entropy is high (regime uncertainty), reduce gamma to flush
     * old transition statistics faster. When stable, increase gamma for memory.
     *
     * Formula: γ_t = γ_base - η × H_t
     *
     * Where H_t = -Σ p(r_t) log p(r_t) is regime entropy.
     *
     * @param sd            Soft Dirichlet structure
     * @param entropy       Current regime entropy (from RBPF out->regime_entropy)
     * @param gamma_base    Base gamma for stable markets (default 0.999)
     * @param eta           Sensitivity to entropy (0.02-0.05 typical)
     *
     * Example:
     *   - Low entropy (0.3): γ → 0.999 - 0.03×0.3 = 0.990 (stable)
     *   - High entropy (1.4): γ → 0.999 - 0.03×1.4 = 0.957 (fast adapt)
     */
    ...
    /**
     * @brief Reset priors from HDP-BEAM posterior
     *
     * Called periodically to synchronize Dirichlet priors with HDP-BEAM's
     * discovered transition structure. Prevents drift between online learning
     * and offline structure discovery.
     *
     * @param sd            Soft Dirichlet structure
     * @param hdp_trans     HDP-BEAM transition matrix [n_regimes × n_regimes]
     * @param confidence    How much to weight HDP prior (1.0-20.0)
     *                      Higher = more trust in HDP, slower online adaptation
     */
