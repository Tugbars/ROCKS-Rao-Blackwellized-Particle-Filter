...
void soft_dirichlet_init(SoftDirichlet *sd, int n_regimes, float ess_max)
{
    ...
    if (n_regimes < 1)
        n_regimes = 1;
    if (n_regimes > SOFT_DIRICHLET_MAX_REGIMES)
        n_regimes = SOFT_DIRICHLET_MAX_REGIMES;
...
void soft_dirichlet_adapt_gamma(SoftDirichlet *sd,
                                float entropy,
                                float gamma_base,
                                float eta)
{
    ...
    /* Clamp inputs */
    gamma_base = maxf(0.9f, minf(0.9999f, gamma_base));
    eta = maxf(0.0f, minf(0.1f, eta));
    entropy = maxf(0.0f, entropy);

    /* γ_t = γ_base - η × H_t */
    float gamma_t = gamma_base - eta * entropy;

    /* Clamp to safe range */
    const float GAMMA_MIN = 0.95f;   /* Don't forget too fast */
    const float GAMMA_MAX = 0.9995f; /* Don't remember forever */

    sd->gamma = maxf(GAMMA_MIN, minf(GAMMA_MAX, gamma_t));
}

void soft_dirichlet_reset_from_hdp(SoftDirichlet *sd,
                                   const float *hdp_trans,
                                   float confidence)
{
    if (!sd || !hdp_trans)
        return;
    ...
    /* Clamp confidence */
    confidence = maxf(1.0f, minf(50.0f, confidence));

    /* Reset alpha to prior derived from HDP posterior
     *
     * α[i][j] = floor + confidence × HDP_trans[i][j]
     *
     * This makes HDP's learned structure the new "prior",
     * while still allowing online adaptation.
     */
    for (int i = 0; i < R; i++)
    {
        float row_sum = 0.0f;
        ...
        for (int j = 0; j < R; j++)
        {
            float hdp_p = hdp_trans[i * R + j];
            hdp_p = maxf(0.0f, minf(1.0f, hdp_p)); /* Sanitize */

            sd->alpha[i][j] = sd->alpha_floor + confidence * hdp_p;
            row_sum += sd->alpha[i][j];
        }

        /* Update row ESS */
        sd->row_ess[i] = row_sum;

        /* Recompute posterior mean for this row */
        if (row_sum > 1e-10f)
        {
    ...
    /* Reset state */
    sd->initialized = 0; /* Will re-initialize prev_probs on next update */
}
...
{
    if (!sd || !regime_probs || !regime_liks)
        return;
...
{
    if (!sd || !regime_probs || !log_regime_liks)
        return;
    ...
    for (int j = 1; j < R; j++)
    {
        if (log_regime_liks[j] > max_ll)
            max_ll = log_regime_liks[j];
    }
...
float soft_dirichlet_prob(const SoftDirichlet *sd, int from, int to)
{
    if (!sd)
        return 0.0f;
    if (from < 0 || from >= sd->n_regimes)
        return 0.0f;
    if (to < 0 || to >= sd->n_regimes)
        return 0.0f;
...
float soft_dirichlet_stickiness(const SoftDirichlet *sd, int regime)
{
    if (!sd)
        return 0.0f;
    if (regime < 0 || regime >= sd->n_regimes)
        return 0.0f;
...
SoftDirichletStats soft_dirichlet_stats(const SoftDirichlet *sd)
{
    ...
    for (int i = 0; i < R; i++)
    {
        ...
        if (sd->row_ess[i] < stats.min_row_ess)
            stats.min_row_ess = sd->row_ess[i];
        if (sd->row_ess[i] > stats.max_row_ess)
            stats.max_row_ess = sd->row_ess[i];
    }
                                ...
                                int n_regimes)
{
    if (!sd || !trans_lut)
        return;
    if (n_regimes > sd->n_regimes)
        n_regimes = sd->n_regimes;
