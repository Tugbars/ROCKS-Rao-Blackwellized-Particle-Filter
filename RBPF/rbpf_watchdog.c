...
{
    if (!weights || n <= 0) return RESET_ZERO_WEIGHTS;
    ...
    for (int i = 0; i < n; i++) {
        if (is_nan_f(weights[i])) has_nan = 1;
        if (is_inf_f(weights[i])) has_inf = 1;
        sum += weights[i];
    }
    
    if (has_nan) return RESET_NAN_WEIGHT;
    if (has_inf) return RESET_INF_WEIGHT;
    if (sum < 1e-30f) return RESET_ZERO_WEIGHTS;
...
{
    if (is_nan_f(h_mean) || is_nan_f(h_var)) return RESET_STATE_NAN;
    if (is_inf_f(h_mean) || is_inf_f(h_var)) return RESET_STATE_NAN;
    if (fabsf(h_mean) > max_abs) return RESET_STATE_EXPLOSION;
...
{
    if (!wd || wd->config.regime_deadlock_window <= 0) return 0;
    ...
    /* Need enough history */
    if (wd->recent_returns_idx < window) return 0;
...
{
    if (!ext || !wd || !wd->config.enabled) return RESET_NONE;
    if (wd->circuit_breaker_tripped) return RESET_NONE;  /* Already dead */
    
    wd->ticks_since_last_reset++;
    
    RBPF_KSC *rbpf = ext->rbpf;
    if (!rbpf) return RESET_NONE;
    ...
    if (ess_ratio < cfg->ess_warning_ratio) {
        ...
        if (wd->consecutive_ess_warnings % 10 == 1) {  /* Rate limit logging */
     ...
     * y = log(r²) ≈ 2*h + noise
     * So implied h ≈ y/2 = log(r²)/2
     *─────────────────────────────────────────────────────────────────────*/
    if (fabsf(obs) > 1e-10f) {  /* Non-trivial observation */
        float y = logf(obs * obs + 1e-20f);  /* log(r²) */
        float h_implied = y / 2.0f;
        ...
        if (divergence_sigma > cfg->state_divergence_sigma) {
            /* Don't trigger immediately on single outlier - 
               could be a legitimate fat tail event */
            static int divergence_count = 0;
            divergence_count++;
            
            if (divergence_count >= 3) {  /* 3 consecutive divergent obs */
                divergence_count = 0;
                        ...
                        RBPF_ResetReason reason, RBPF_ResetEvent *event)
{
    ...
    case SEVERITY_SOFT:
         ...
         *─────────────────────────────────────────────────────────────────*/
        {
            ...
            /* Add jitter to states */
            float jitter_scale = 0.1f * sqrtf(fmaxf(h_var_before, 0.01f));
            for (int i = 0; i < n; i++) {
                /* Simple jitter - could use better RNG */
                float jitter = jitter_scale * (2.0f * ((float)rand() / RAND_MAX) - 1.0f);
                rbpf->h[i] += jitter;
            }
    ...
    case SEVERITY_MEDIUM:
         ...
         *─────────────────────────────────────────────────────────────────*/
        {
            ...
            for (int i = 0; i < n; i++) {
                ...
                rbpf->h[i] = h_center + h_std * z;
                rbpf->P[i] = h_std * h_std;
                rbpf->weight[i] = w_uniform;
            }
            
            /* Spread regimes (don't lock all particles in one regime) */
            int regimes_per_particle = rbpf->n_regimes;
            for (int i = 0; i < n; i++) {
                rbpf->regime[i] = i % regimes_per_particle;
            }
        }
        break;
    ...
    case SEVERITY_CRITICAL:
         ...
         * - Optionally clear learned parameters
         *─────────────────────────────────────────────────────────────────*/
        {
            /* Full state reset */
            ...
            for (int i = 0; i < n; i++) {
                ...
                rbpf->h[i] = h_init + sqrtf(P_init) * z;
                rbpf->P[i] = P_init;
                rbpf->weight[i] = w_uniform;
                rbpf->regime[i] = i % rbpf->n_regimes;
            }
            
            /* Clear parameters if configured */
            if (!cfg->preserve_learned_params && ext->storvik_initialized) {
                /* Would need to re-initialize Storvik here */
                /* For now, just log warning */
                fprintf(stderr, "[WATCHDOG] Warning: Parameter reset not implemented\n");
            }
        }
        break;
...
{
    if (reason < 0 || reason >= RESET_REASON_COUNT) return "UNKNOWN";
    return REASON_STRINGS[reason];
}

const char* rbpf_watchdog_severity_str(RBPF_ResetSeverity severity)
{
    if (severity < 0 || severity > SEVERITY_CRITICAL) return "UNKNOWN";
