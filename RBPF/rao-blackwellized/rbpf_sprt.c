 ...
 *
 * Session 12 Update: Replaced [0,1] probability averaging with log-odds consensus.
 * This is proper Bayesian evidence combination: P(k|data) ∝ exp(Σ log_ratio_{k vs j})
 */
...
SPRT_Decision sprt_binary_update(SPRT_Binary *sprt, double ll_h1, double ll_h0)
{
     ...
     * This is the SPRT equivalent of the KL ceiling - prevents
     * a single tick from moving evidence by more than 20 nats */
    if (delta > 20.0)
        delta = 20.0;
    if (delta < -20.0)
        delta = -20.0;
                     ...
                     double alpha, double beta, int min_dwell)
{
    if (n_regimes < 2)
        n_regimes = 2;
    if (n_regimes > SPRT_MAX_REGIMES)
        n_regimes = SPRT_MAX_REGIMES;
    ...
    sprt->dwell_bypass_active = 0; /* No bypass by default */
...
/**
 * @brief Compute log-odds consensus for regime k
 *
 * Sum of pairwise log-ratios favoring regime k against all others.
 * This is the proper Bayesian combination: P(k|data) ∝ exp(Σ log_ratio_{k vs j})
 *
 * Working in log-space avoids the information loss of mapping to [0,1] and averaging.
 */
static double compute_regime_log_consensus(const SPRT_Multi *sprt, int k)
{
    double log_sum = 0.0;
    ...
    for (int j = 0; j < n; j++)
    {
        ...
        int i_lo = (k < j) ? k : j;
        int i_hi = (k < j) ? j : k;
        double lr = sprt->tests[i_lo][i_hi].log_ratio;

        /* log_ratio > 0 means evidence for i_hi (higher index)
         * log_ratio < 0 means evidence for i_lo (lower index)
         *
         * If k is the higher index, positive lr favors k
         * If k is the lower index, negative lr favors k (flip sign)
         */
        log_sum += (k == i_hi) ? lr : -lr;
    }
...
/**
 * @brief Aggregate pairwise evidence via log-odds consensus
 *
 * For each regime k, compute its total log-evidence against the field,
 * then apply softmax to get normalized probabilities.
 *
 * This replaces the old [0,1] averaging which lost information.
 */
static void aggregate_evidence(SPRT_Multi *sprt)
{
    int n = sprt->n_regimes;
    double log_ev[SPRT_MAX_REGIMES];
    double max_log_ev = -1e30;

    /* Compute log-consensus for each regime */
    for (int k = 0; k < n; k++)
    {
        log_ev[k] = compute_regime_log_consensus(sprt, k);
        if (log_ev[k] > max_log_ev)
        {
            max_log_ev = log_ev[k];
        }
    }

    /* Softmax normalization: P(k) = exp(log_ev[k] - max) / Σ exp(log_ev[j] - max) */
    double sum_exp = 0.0;
    for (int k = 0; k < n; k++)
    {
        sum_exp += exp(log_ev[k] - max_log_ev);
    }

    /* Store normalized probabilities */
    double inv_sum = (sum_exp > 1e-30) ? 1.0 / sum_exp : 1.0;
    for (int k = 0; k < n; k++)
    {
        sprt->regime_evidence[k] = exp(log_ev[k] - max_log_ev) * inv_sum;
    }
}
...
int sprt_multi_update(SPRT_Multi *sprt, const double *log_liks)
{
    ...
    /* Aggregate evidence using log-odds consensus */
    aggregate_evidence(sprt);

    /* Check if we should switch (respecting dwell time unless bypassed) */
    if (sprt->ticks_in_current < sprt->min_dwell_time &&
        !sprt->dwell_bypass_active)
    {
        return current; /* Too soon to switch */
    }

    /* Clear bypass flag after use */
    sprt->dwell_bypass_active = 0;
     ...
     * With log-odds aggregation, this is more meaningful - it represents
     * the regime that beats all others in pairwise comparisons */
...
{
    if (regime < 0 || regime >= sprt->n_regimes)
        return;
...
void sprt_multi_set_dwell_bypass(SPRT_Multi *sprt, int active)
{
    if (sprt)
    {
        sprt->dwell_bypass_active = active;
    }
}
...
static double log_gamma(double x)
{
    ...
    if (x > 10)
    {
        return (x - 0.5) * log(x) - x + 0.5 * log(2.0 * M_PI) + 1.0 / (12.0 * x) - 1.0 / (360.0 * x * x * x);
    }
...
double sprt_student_t_loglik(double y, double mu, double var, double nu)
{
     ...
     * - ((ν+1)/2) * log(1 + (y-μ)²/(ν*σ²))
     */
    double log_lik = log_gamma((nu + 1.0) / 2.0) - log_gamma(nu / 2.0) - 0.5 * log(nu * M_PI * scale_sq) - ((nu + 1.0) / 2.0) * log(1.0 + diff * diff / (nu * scale_sq));
...
double sprt_logchisq_loglik(double y_log_sq, double h)
{
    ...
    for (int k = 0; k < 10; k++)
    {
        ...
        double log_lik = log(OCSN_PI[k]) - 0.5 * log(2.0 * M_PI * OCSN_VAR[k]) - 0.5 * diff * diff / OCSN_VAR[k];
        log_liks[k] = log_lik;
