...
int rbpf_ksc_resample(RBPF_KSC *rbpf)
{
     ...
     *
     * After blind Silverman scatter, apply Metropolis-Hastings refinement:
     *   - Proposes moves toward higher likelihood
     *   - Rejects moves that cross regime boundaries (Storvik safe)
     *   - Auto-tunes proposal scale to ~50% acceptance
     *
     * Turns random scatter into likelihood-guided exploration.
     * ═══════════════════════════════════════════════════════════════════════ */
    if (rbpf->mh_jitter.enabled)
    {
#ifndef RBPF_USE_DOUBLE
        /* Single precision: cast mu to float* */
        mh_jitter_apply(&rbpf->mh_jitter,
                        (float *)rbpf->mu,
                        rbpf->regime,
                        n,
                        (float)rbpf->last_y,
                        ...
                        rbpf->mkl_rng[0]);
#else
        /* Double precision: need a temporary float buffer
         * (MH jitter uses float internally for efficiency) */
        float *mu_f = (float *)mkl_malloc(n * sizeof(float), 64);
        if (mu_f)
        {
            for (int i = 0; i < n; i++)
                mu_f[i] = (float)rbpf->mu[i];
