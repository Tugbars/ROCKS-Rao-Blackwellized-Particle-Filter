...
/* ═══════════════════════════════════════════════════════════════════════════
 * TIMING - Cross-platform cycle counter
 * ═══════════════════════════════════════════════════════════════════════════ */
...
static int test_volume_events(void)
{
    ...
    for (int i = 0; i < 10000; i++)
    {
        ...
        if (evt.last_return_event)
            return_events++;
        if (evt.last_volume_event)
            volume_events++;
    }
...
static int test_imbalance_events(void)
{
    ...
    {
        double obs = sigma * randn() * 0.5; /* Small returns */
        double imbalance = 0.05 * randn();  /* Small imbalance (keeps EMA low) */

        /* Every 100 ticks, spike imbalance */
        bool is_spike = (i % 100 == 50 && i > 100);
        if (is_spike)
        {
    ...
    printf("  Total imbalance events: %d\n", imbalance_events);
    printf("  Spike-triggered events: %d (expected ~99)\n", spike_events);

    /* Most events should be from spikes */
    int pass = (spike_events > 80 && spike_events <= 99);
    printf("Result: %s\n", pass ? "PASS" : "FAIL");
