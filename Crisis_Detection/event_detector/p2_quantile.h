...
/* Portable NAN for MSVC and others */
#ifndef P2_NAN
#if defined(_MSC_VER)
/* MSVC: use quiet NaN via union */
static __inline double p2_get_nan(void)
{
    union
    {
        unsigned __int64 i;
...
#define P2_NAN p2_get_nan()
#elif defined(NAN)
#define P2_NAN ((double)NAN)
#else
/* Fallback: generate at runtime */
#define P2_NAN (0.0 / 0.0)
#endif
#endif

/* Portable inline for MSVC */
#if defined(_MSC_VER) && !defined(__cplusplus)
#define P2_INLINE static __inline
#else
#define P2_INLINE P2_INLINE
#endif
...
extern "C"
{
     ...
     */
    P2_INLINE void p2_init(P2Quantile *pq, double p)
    {
     ...
     */
    P2_INLINE double p2_linear(double q0, double q1, int n0, int n1, int d)
    {
     ...
     */
    P2_INLINE void p2_update(P2Quantile *pq, double x)
    {
        pq->count++;
     ...
     */
    P2_INLINE double p2_get_quantile(const P2Quantile *pq)
    {
        if (!pq->initialized)
        {
            return (pq->count > 0) ? pq->q[pq->count - 1] : P2_NAN;
        }
     ...
     */
    P2_INLINE int p2_get_count(const P2Quantile *pq)
    {
        return pq->count;
    }
     ...
     */
    P2_INLINE int p2_is_initialized(const P2Quantile *pq)
    {
     ...
     */
    P2_INLINE void p2_reset(P2Quantile *pq)
    {
        double p = pq->p;
        p2_init(pq, p);
    }
     ...
     */
    P2_INLINE void p2_get_markers(const P2Quantile *pq,
