...
float sr_accumulate_step(float log_lr, float log_sr_old, float clamp) {
    ...
    float log_one_plus_R = log1pf(expf(log_sr_old));
    float log_sr_new = log_lr + log_one_plus_R;
    
    /* Clamp to prevent overflow */
    log_sr_new = fmaxf(-clamp, fminf(log_sr_new, clamp));
...
float dual_sr_update_exit(DualSR *dsr, float obs) {
    ...
    dsr->sigma_crisis = (1.0f - dsr->sigma_crisis_ema_alpha) * dsr->sigma_crisis 
                      + dsr->sigma_crisis_ema_alpha * abs_obs;
...
void sr_compute_log_lr_batch(
    ...
    vsSubI(n, z0_sq, 1, z1_sq, 1, log_lr, 1);
    cblas_sscal(n, 0.5f, log_lr, 1);
    vsAdd(n, log_lr, log_sigma_ratio, log_lr);
