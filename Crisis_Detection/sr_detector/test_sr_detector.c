...
/* MSVC on Windows */
#include <malloc.h>
#define test_aligned_alloc(align, size) _aligned_malloc((size), (align))
#define test_aligned_free(ptr) _aligned_free(ptr)
#else
/* GCC/Clang on Linux/Mac */
#define test_aligned_alloc(align, size) aligned_alloc((align), (size))
#define test_aligned_free(ptr) free(ptr)
#endif
...
static int test_batch_performance(void)
{
    ...
    const int N = 100000;
    float *obs = (float *)test_aligned_alloc(64, N * sizeof(float));
    float *log_lr = (float *)test_aligned_alloc(64, N * sizeof(float));
    float *log_sr = (float *)test_aligned_alloc(64, N * sizeof(float));
    ...
    test_aligned_free(obs);
    test_aligned_free(log_lr);
    test_aligned_free(log_sr);
...
static int test_student_t_consistency(void)
{
    ...
    const int N = 1000;
    float *obs = (float *)test_aligned_alloc(64, N * sizeof(float));
    float *log_lr_batch = (float *)test_aligned_alloc(64, N * sizeof(float));
    float *log_lr_scalar = (float *)test_aligned_alloc(64, N * sizeof(float));
    ...
    test_aligned_free(obs);
    test_aligned_free(log_lr_batch);
    test_aligned_free(log_lr_scalar);
